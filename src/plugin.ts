...
export async function parseMarkdown(path: string, remark: UnifiedPlugin[], rehype: UnifiedPlugin[]): Promise<VFile> {
...
export default function plugin<Schema extends z.Schema>(options: Options<Schema>): Plugin {
  return {
    ...
    async transform(content, file): Promise<TransformResult> {
      ...
      const { data: { matter }, value: body } = await parseMarkdown(file, options.remark || [], options.rehype || [])
      ...
      if (attributes) {
        constants = Object.entries(attributes).map(([key, value]) => `export const ${key} = "${value}"`)
      }
